...
public class Test {
    ...
    public static void task12() throws IOException {
        ...
        var listener = new CPLangParserBaseListener() {
			...
			@Override
			public void exitVariableDef(CPLangParser.VariableDefContext ctx) {
				varDefs++;
			}
			...
			@Override
			public void exitFunctionDef(CPLangParser.FunctionDefContext ctx) {
				funcDefs++;
			}
		};
    ...
    public static void task3() throws IOException {
		...
		var visitor = new CPLangParserBaseVisitor<Integer>() {
			...
			@Override
			public Integer visitPlusExpr(CPLangParser.PlusExprContext ctx) {
				return visit(ctx.expr(0)) + visit(ctx.expr(1));
			}

			@Override
			public Integer visitMultExpr(CPLangParser.MultExprContext ctx) {
				return visit(ctx.expr(0)) * visit(ctx.expr(1));
			}

			@Override
			public Integer visitDivExpr(CPLangParser.DivExprContext ctx) {
				return visit(ctx.expr(0)) / visit(ctx.expr(1));
			}

			@Override
			public Integer visitParanExpr(CPLangParser.ParanExprContext ctx) {
