...
subset *computeSubsets() {
    ...
    int workerSubsetsLen = 0;
    omp_set_num_threads(p);

    #pragma omp parallel private(threadSubsets) shared(workerSubsets, workerSubsetsLen)
    {
        int len = 0;
        threadSubsets = malloc(((workerEndRank - workerStartRank) / p) * sizeof(subset));
        #pragma omp for
            // compute subset for every rank
            for (int i = workerStartRank; i < workerEndRank; i++)
            {
        ...
        #pragma omp critical
            { // gather all computed subsets in the final vector of subsets(allSUbsets)
                for(int i = 0; i < len; i++) {
                    workerSubsets[workerSubsetsLen++] = threadSubsets[i];
                }
            }
    }
