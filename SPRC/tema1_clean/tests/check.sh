...
SERVER_NAME="server"
SERVER_PARAMS="tests/test$1/userIDs.db tests/test$1/resources.db tests/test$1/approvals.db $(cat tests/test$1/readme)"
CLIENT_NAME="client"
...
if [[ $1 =~ $numberPattern ]]; then
    ...
    do
        SERVER_NAME="server"
        SERVER_PARAMS="tests/test$i/userIDs.db tests/test$i/resources.db tests/test$i/approvals.db $(cat tests/test$i/readme)"
        CLIENT_NAME="client"
